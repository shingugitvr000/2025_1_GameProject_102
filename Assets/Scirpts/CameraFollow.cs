...
public class CameraFollow : MonoBehaviour
{
    ...
    private void LateUpdate()                                               //카메라 움직음은 보통 LateUpdate에서 처리
    {
        //LateUpdate를 사용하는 이유는 카메라가 플레이어의 이동을 모두 처리한 이후에 따라가게 하기 위해 
        ...
        transform.LookAt(target.position);                                                           //카메라가 항상 플레이어를 바라도록 설정
    }
}
